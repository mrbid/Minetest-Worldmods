...
for name, info in pairs(wrench.registered_nodes) do
	...
	if olddef then
		...
		newdef.stack_max = 2
		if string.find(newdef.description, "Protected") or string.find(newdef.description, "Locked") then
			newdef.description = S("%s with items"):format(newdef.description)
		elseif string.find(newdef.description, "Amethyst") then
			-- local i, j = string.find(newdef.description, " ")
			-- local t = string.sub(newdef.description, 0, i)
			newdef.description = S("Amethyst Mystery Box"):format(newdef.description)
		elseif string.find(newdef.description, "Diamond") then
			newdef.description = S("Diamond Gift Box"):format(newdef.description)
		elseif string.find(newdef.description, "Titanium") then
			newdef.description = S("Titanium Mystery Box"):format(newdef.description)
		elseif string.find(newdef.description, "Gold") then
			newdef.description = S("Gold Mystery Box"):format(newdef.description)
		elseif string.find(newdef.description, "Silver") then
			newdef.description = S("Silver Mystery Box"):format(newdef.description)
		elseif string.find(newdef.description, "Mithril") then
			newdef.description = S("Mithril Mystery Box"):format(newdef.description)
		elseif string.find(newdef.description, "Copper") then
			newdef.description = S("Copper Mystery Box"):format(newdef.description)
		elseif string.find(newdef.description, "Iron") then
			newdef.description = S("Iron Mystery Box"):format(newdef.description)
		else
			newdef.description = S("%s with items"):format(newdef.description)
		end
		newdef.groups = {}
