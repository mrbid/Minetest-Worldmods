...
local add_inventory_to_account = function(market, account, item, quantity)
	if string.find(item, "geld") or string.find(item, "bookshelf") then
		minetest.chat_send_player(account.name, "Minegeld is prohibited.");
		return false;
	end
...
commoditymarket.register_market = function(market_name, market_def)
	...
	local inv = minetest.create_detached_inventory("commoditymarket:"..market_name, {
		...
		allow_put = function(inv, listname, index, stack, player)
			...
			-- dont allow minegeld or evil bookshelf >:(
			if string.find(item, "geld") or string.find(item, "bookshelf") then
				minetest.chat_send_player(player:get_player_name(), "Minegeld is prohibited.");
				return 0
			end
