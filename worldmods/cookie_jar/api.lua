-- Cookie Jar
-- by Grizzly Adam
-- Inventroy Shelf code by Zorman2000
...
local function update_shelf(pos)
	-- Remove all objects
	local objs = minetest.get_objects_inside_radius(pos, 1.75)
	...
	local horizontal_displacement = 0.375
	...
	if obj_count > 0 then
		...
		for i = 1, shown_items do
			...
			if i == ((shown_items / 2) + 1) then
				start_pos.y = start_pos.y - 0.25 --set Y seperation
			end
			local item_displacement = 0.25 --set X seperation
			if shown_items == 6 then
				item_displacement = 0.2775 --set X seperation
			end
			local obj_pos = {
			...
			if not list[i]:is_empty() then
				...
				temp_size = 0.46/max_shown_items -- size on load, make match size when placed
				--minetest.log("Size: "..dump(temp_size))
				local ent = minetest.add_entity(obj_pos, "cookie_jar:item")
				ent:set_properties({
					wield_item = temp_texture,
					visual_size = {x = 0.46/max_shown_items, y = 0.46/max_shown_items}
				})
...
function itemshelf.register_shelf(name, def)
	...
	minetest.register_node("cookie_jar:"..name, {
		...
		on_construct = function(pos)
		 ...
		 allow_metadata_inventory_put = function(pos, listname, index, stack, player)
		 	if minetest.get_item_group(stack:get_name(), "confection") ~= 0 then
...
-- Entity for shelf
minetest.register_entity("cookie_jar:item", {
	hp_max = 1,
	visual = "wielditem",
