...
local truncate_string = function(target, length)
	if string.len(target) > length then
		return string.sub(target,1,length-2).."..."
	end
	return target
end
...
local get_item_description = function(item)
	...
	if def then
		...
		if description then
			if string.len(def.description) > truncate_item_names_to then
				description = def.name;
				description = description:gsub(".*:", "")
				description = description:gsub('%W', ' ')
				description = description:gsub("(%l)(%w*)", function(a,b) return string.upper(a)..b end)
			end
			return minetest.formspec_escape(truncate_string(description, truncate_item_names_to))
		end
	end
...
local get_account_formspec = function(market, account)
	...
	for i, entry in ipairs(inventory) do
		...
		formspec[#formspec+1] = "," .. truncate_string(entry.description, truncate_item_names_to) .. "," .. entry.quantity
	end	
