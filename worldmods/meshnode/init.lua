...
minetest.register_entity("meshnode:ctrl", {
	...
	set_alignment = function(self)
		...
		self.object:set_velocity({x=0, y=0, z=0})
		self.object:set_yaw(math.rad(deg))
...
minetest.register_node("meshnode:controller", {
	...
	allow_metadata_inventory_put = function(pos, listname, index, stack, player)
		...
		if meta then
			local v1 = minetest.string_to_pos(meta);
			if v1 ~= nil then
			...
			if pos then
				...
				if node then
					if string.find(node.name, "meshnode:scaffold") then
						...
						local dist = vector.distance(parent.object:get_pos(), pos)
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname then
		...
		elseif fields.attach and entity.player == nil then
			player:set_attach(entity.object, "", {x=0, y=6, z=0}, {x=0, y=0, z=0})
		...
		elseif fields.detach and entity.player == player then
			...
			minetest.after(0.1, function()
				player:set_pos({x=p.x, y=p.y + 1, z=p.z})
			end)
